...
class LedgerService {
   ...
   * @returns
   */
  async logLedgerCreditEntry(
    tranx,
    { walletId, transactionId, credit = 0, prevBalance = 0, currBalance = 0 }
  ) {
    try {
      ...
      return await this.logLedgerEntry(tranx, {
        walletId,
        transactionId,
        amount: credit,
        prevBalance,
        currBalance,
        type: TransactionFlow.Credit,
      });
    } catch (error) {
   ...
   * @returns
   */
  async logLedgerDebitEntry(
    tranx,
    { walletId, transactionId, debit = 0, prevBalance = 0, currBalance = 0 }
  ) {
    try {
      if (!walletId) throw new Error("Invalid wallet ID");
      if (!transactionId) throw new Error("Invalid transaction ID");
  ...
  async logLedgerEntry(
    ...
    // Validate entry type
    if (!Object.values(TransactionFlow).includes(type))
      throw new Error("Invalid entry type");

    // Check if ledger entry already exists for this transaction and wallet
    const existingEntry = await tranx.ledger.findFirst({
      where: { walletId, transactionId, isDeleted: false },
    });
    if (existingEntry) return existingEntry;

    // Create ledger entry
    const ledgerEntry = await tranx.ledger.create({
      data: {
        walletId,
        transactionId,
        prevBalance,
   ...
   */
  async validateLedgerInflowOutflowConsistency(tranx, { walletId }) {
    try {
      if (!walletId) throw new Error("Invalid wallet ID");

      // Get user wallet
      const wallet = await tranx.wallet.findUnique({ where: { id: walletId } });
      if (!wallet) throw new Error("Wallet not found");

      // Use Prisma's aggregation to calculate total credits and debits
      const ledgerSummary = await tranx.ledger.aggregate({
      ...
      // Extract the sums (will be 0 if no entries found)
      const totalCredits = ledgerSummary._sum.credit || 0;
      const totalDebits = ledgerSummary._sum.debit || 0;
      ...
      if (expectedBalance !== wallet.balance) {
        console.error(
          `Ledger inconsistency detected for wallet ${walletId}: expected balance ${expectedBalance}, actual balance ${wallet.balance}`
        );
      ...
      console.error(
        `[LedgerService][validateLedgerInflowOutflowConsistency]`,
        error
      );
      return false;
    }
