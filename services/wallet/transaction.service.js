...
class TransactionService {
  ...
  async createTransaction(
    ...
    try {
      ...
      const existingTransaction = await tranx.transaction.findFirst({
        where: { reference, isDeleted: false },
      });
      if (existingTransaction)
        throw new Error("A transaction with this reference already exist.");
  ...
  async getTransactionByReference(reference) {
    try {
      ...
      return await prismadb.transaction.findFirst({
        where: { reference, isDeleted: false },
      });
    } catch (error) {
