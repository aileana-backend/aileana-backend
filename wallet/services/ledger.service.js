...
class LedgerService {
	 ...
	 * @returns
	 */
	async logLedgerCreditEntry(tranx, { walletId, transactionId, credit = 0, prevBalance = 0, currBalance = 0 }) {
		try {
	 ...
	 * @returns
	 */
	async logLedgerDebitEntry(tranx, { walletId, transactionId, debit = 0, prevBalance = 0, currBalance = 0 }) {
		try {
			if (!walletId) throw new Error("Invalid wallet ID")
			if (!transactionId) throw new Error("Invalid transaction ID")
