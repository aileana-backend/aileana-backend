	...
	try {
		...
		switch (eventType) {
			...
			case "SUCCESSFUL_TRANSACTION":
				// Get user ID
				const userId = eventData.product?.reference?.replace("AILEANA_", "") || null
				if (!userId) {
					return res.status(400).json({ msg: "Invalid user reference in webhook data" })
				}
				...
				const existingTransaction = await transactionService.getTransactionByReference(eventData.transactionReference)
				if (existingTransaction) {
					return res.status(200).json({ msg: "Transaction already processed" })
				}

				// Credit user wallet
				const charges = Number(eventData?.amountPaid) - Number(eventData?.settlementAmount)
				const toBeCredited = Number(eventData?.settlementAmount)
				const creditTrnx = await walletService.creditWallet(userWallet.id, toBeCredited, charges, eventData?.transactionReference, "Wallet Top-up", {
