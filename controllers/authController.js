...
const signup = async (req, res) => {
	try {
		const { username, first_name, middle_name, last_name, email, dob, gender, password, biometricPreference, termsAccepted } = req.body
...
const resetForgotPassword = async (req, res) => {
	try {
		...
		if (!user.resetVerified || !user.resetVerifiedExpires || Date.now() > user.resetVerifiedExpires) {
...
const biometricLogin = async (req, res) => {
	try {
		...
		if (!email) {
			return res.status(400).json({ message: "Email is required for biometric login" })
		}
		...
		if (user.biometricPreference === "None") {
			return res.status(403).json({ message: "Biometric login is not enabled for this user" })
		}
...
const verifyAccountOtp = async (req, res) => {
	try {
		...
		// Check OTP validity
		if (!user.otp || user.otpType !== "signup" || Date.now() > user.otpExpires) {
...
const toggleSmartReply = async (req, res) => {
	try {
		...
		const user = await User.findByIdAndUpdate(userId, { smartReplyEnabled: enabled }, { new: true, select: "username email smartReplyEnabled" })
